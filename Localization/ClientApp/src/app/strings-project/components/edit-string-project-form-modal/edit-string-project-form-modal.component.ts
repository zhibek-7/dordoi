...
export class EditStringProjectFormModalComponent extends ModalComponent implements OnInit {
  ...
  constructor(private translationSubstringService: TranslationSubstringService,
  ...
  getSourceLocale() {
    this.languageService.getSourceLocaleLocalizationProject(this.projectsService.currentProjectId)
      .subscribe(
        locale => {
          this.translationSubstring.localization_project_source_locale_name = locale.name_text;
        },
      error => {
        console.error(error);
      }
  ...
  getTranslation() {
    this.translationService.getAllTranslationsInStringWithLocaleById(this.translationSubstring.id)
      .subscribe(
      translations => {
  ...
  saveTranslationSubstring() {
    ...
    this.translationSubstringService.saveSubstringToTranslate(editedTranslationSubstring)
      .subscribe(
        () => {},
  ...
  saveTranslation() {
    this.translationService.saveTranslated(this.translationSubstring.translations)
      .subscribe(
        () => {},
        error => { console.error(error); }
      );
